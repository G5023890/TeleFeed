...
- Clicking a notification opens TeleFeed directly to the unread list of the corresponding channel.
...
```bash
open TeleFeed.xcodeproj
```

## Telegram Credentials Setup

TeleFeed does not hardcode `api_id` or `api_hash`.
...
5. TeleFeed stores them in the macOS Keychain.
...
- Persistent state directory in `~/Library/Application Support/TeleFeed/TDLibState`
- Temporary media cache in `~/Library/Caches/TeleFeed/TDLibTempMedia`
...
2. TeleFeed initializes TDLib and requests QR authentication.
...
8. Opening a post in TeleFeed marks it as viewed/read through TDLib for that Telegram account.
